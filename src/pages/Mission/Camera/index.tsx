...
export default function MissionCamera() {
  const { placeId, image, latitude, longitude } = useLocalSearchParams<{
    placeId?: string;
    image?: string;
    name?: string;
    latitude?: string;
  ...
  const handleCapture = async () => {
    ...
    if (targetPlaceId === null || targetLatitude === null || targetLongitude === null) {
      Alert.alert('위치 정보 없음', '이 장소의 위치 정보를 확인할 수 없어요.');
      return;
    }
    ...
    try {
      ...
      if (status !== 'granted') {
        Alert.alert('위치 권한 필요', '미션 인증을 위해 위치 접근 권한이 필요합니다.');
        return;
      }
      ...
      if (distance > MISSION_SUCCESS_RADIUS_METERS) {
        Alert.alert(
          '미션 실패',
          `장소에서 약 ${Math.round(distance)}m 떨어져 있어요. ${MISSION_SUCCESS_RADIUS_METERS}m 이내에서 다시 촬영해주세요.`,
        );
        return;
      }
    ...
    <View className="flex-1 bg-black">
      ...
      <SafeAreaView style={StyleSheet.absoluteFill} pointerEvents="box-none">
        <View className="flex-row items-start justify-between px-16 pt-8">
          <Pressable onPress={() => router.back()} hitSlop={12} className="p-8">
